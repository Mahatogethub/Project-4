...
const shortenUrl = async (req, res) => {
  try {
    ...
    await axios
      .get(url.longUrl)
      .then((res) => {
        if (res.status == 200 || res.status == 201) Link = true;
      })
      .catch((error) => {
        Link = false;
      });
    //  == only check value whereas === check both value and data type
    if (Link == false)
    ...
    if (presentUrl) {
      return res.status(200).send({
        status: true,
        message: "already created(cache)",
        data: JSON.parse(presentUrl),
      });
    }
    ...
    if (existUrl) {
      ...
      return res
        .status(200)
        .send({ status: true, message: "already created(DB)", data: existUrl });
    }
